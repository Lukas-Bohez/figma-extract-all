// Figma Extract All v13.0.0 — Full Lottie import into Figma
...
function analyzeLottieFile(fileName:string,content:string):LottieAnalysis{
  ...
  analysis.meta.frameRate=json.fr||0;analysis.meta.inPoint=json.ip||0;analysis.meta.outPoint=json.op||0;
  analysis.meta.width=json.w||0;analysis.meta.height=json.h||0;
  ...
  const bm=analysis.bodymovinSettings;bm.compressedJson=typeof json.ddd==="undefined"&&typeof json.layers!=="undefined";
  if(json.assets){bm.includeAssets=true;analysis.hasAssets=true;
    for(const a of json.assets){const asset:AssetInfo={id:a.id||"",type:"image",name:a.nm||a.p||"unnamed",embedded:!!a.e,refId:a.p||a.u||"",width:a.w,height:a.h};if(a.layers)asset.type="precomp";analysis.assets.push(asset);if(asset.type==="image")analysis.stats.images++;if(asset.type==="precomp")analysis.stats.precomps++;}
  }
  if(json.layers){
    function walkLayers(layers:any[],depth:number):any[]{
      const tree:any[]=[];
      for(const l of layers){
        analysis.stats.layers++;const ty=l.ty;
        ...
        function scanExp(o:any){if(!o||typeof o!=="object")return;if(o.a===1&&o.k&&typeof o.k==="string"&&o.k.length>0){analysis.expressions.push(o.k);bm.includeExpressions=true;}for(const k of Object.keys(o)){if(typeof o[k]==="object")scanExp(o[k]);}}
        scanExp(l);
        const node:any={name:l.nm||"unnamed",type:["precomp","solid","image","null","shape","text"][ty]||"unknown",visible:!(l.hd),hidden:!!l.hd,children:[],ty:ty,raw:l};
        if(l.hd)bm.hiddenLayers=true;if(l.layers)node.children=walkLayers(l.layers,depth+1);tree.push(node);
      }
      return tree;
    }
  ...
  if(!bm.includeAssets)analysis.warnings.push({type:"assets",message:"No image assets found. Images won't appear in Figma.",fix:"In Bodymovin: enable 'Include in json' > 'Assets'"});
  if(!bm.includeKeyframes&&analysis.stats.layers>0)analysis.warnings.push({type:"keyframes",message:"No keyframe data — motion won't be visible.",fix:"In Bodymovin: enable 'Keyframe Data'"});
  if(!bm.includeExpressions&&analysis.expressions.length>0)analysis.warnings.push({type:"expressions",message:"Expressions found — Figma doesn't support AE expressions.",fix:"Bake expressions to keyframes in AE first."});
  if(bm.hiddenLayers)analysis.warnings.push({type:"hidden",message:"Hidden layers detected — will not be visible.",fix:"Unhide layers in AE or use 'Visible layers only'."});
  if(!bm.compressedJson&&json.comp)analysis.warnings.push({type:"compressed",message:"Compressed JSON format — Figma needs uncompressed.",fix:"Uncheck 'Compress JSON' in Bodymovin."});
  if(!analysis.valid)analysis.errors.push("Not a valid Bodymovin/Lottie JSON export.");
  return analysis;
}
...
function buildFullExtractSync(onProgress?:(p:FullExtractProgress)=>void):FullExtract{
  const scope=getScope();const rpt=(s:number,l:string,d:string)=>{if(onProgress)onProgress({step:s,totalSteps:3,label:l,detail:d});};
  rpt(0,"Starting","Scanning...");const allDeepNodes=deepFlatten(scope.roots);
  ...
  const comps=extractAllComponents();const pages=extractPages();const hierarchy:any[]=[];for(const r of scope.roots)hierarchy.push(...buildHierarchy(r));
  ...
  for(const r of scope.roots)cn(r);rpt(3,"Done",`${texts.length} texts, ${totalN} nodes`);
...
function flushZipChunked(fileNameBase:string){
  if(zipFiles.length===0)return;const zip=makeZip(zipFiles);zipFiles=[];
  const CHUNK=400000;const total=Math.ceil(zip.length/CHUNK);const zipName=`${sanitizeName(fileNameBase)}_extract.zip`;
...
figma.ui.onmessage=async(msg:any)=>{
  ...
  // ═══ PLACE ANIMATION IN FIGMA ═══
  if(msg.type==="place-lottie"){
    if(!lastAnalysis||!lastAnalysis.valid){figma.ui.postMessage({type:"error",message:"No valid animation to place. Import a Lottie file first."});return;}
    try{
      placeAnimationInFigma(lastAnalysis);
    }catch(e){
      figma.ui.postMessage({type:"error",message:"Failed to place animation: "+(e as Error).message});
    }
  }

  // ═══ FULL EXTRACT ═══
  if(msg.type==="get-full-extract"&&!msg.aeOpts&&!msg.aiOpts){
    const data=buildFullExtractSync((p)=>{figma.ui.postMessage({type:"full-extract-progress",progress:p});});if(cancelRequested)return;
  ...
  if(msg.type==="get-full-extract"&&msg.aeOpts){
    ...
    figma.ui.postMessage({type:"full-extract",data:{textNodes:data.textNodes.length,variables:0,styles:0,components:data.components.length,totalNodes:data.nodeCounts.total,scope:data.meta.scopeDescription}});flushZipChunked(baseName);
  }
  ...
  if(msg.type==="get-full-extract"&&msg.aiOpts){
    ...
    addToZip(`${sanitizeName(baseName)}_full-extract.json`,JSON.stringify(data,null,2));addToZip(`${sanitizeName(baseName)}_text.txt`,buildPlainText(data.textNodes));
    fetchAndSendVars();fetchAndSendStyles();
    if(msg.aiOpts.includeSVGs&&!cancelRequested){const an=deepFlatten(scope.roots).filter(n=>n.type!=="TEXT"&&(n.type as string)!=="PAGE"&&isVisible(n));for(let i=0;i<an.length;i+=4){if(cancelRequested)break;const br=await Promise.all(an.slice(i,i+4).map(n=>exportNodeSVG(n.id)));for(const r of br){if(r)zipFiles.push(r);}figma.ui.postMessage({type:"progress",current:Math.min(i+4,an.length),total:an.length,label:"SVGs"});}}
    figma.ui.postMessage({type:"full-extract",data:{textNodes:data.textNodes.length,variables:0,styles:0,components:data.components.length,totalNodes:data.nodeCounts.total,scope:data.meta.scopeDescription}});flushZipChunked(baseName);
  }
  ...
  if(msg.type==="export-selected-svg"||msg.type==="export-selected-png"||msg.type==="export-selected-jpg"){
    ...
    const results=await exportNodes(sel.map((n:SceneNode)=>n.id),fmt,sc);if(results.length<=3){if(results.length>0)figma.ui.postMessage({type:"export-results",results});}else{for(const x of results)addToZip(`${x.name}.${x.format.toLowerCase()}`,new Uint8Array(x.bytes));flushZipChunked(baseName+"_selected");}
  }
  ...
  if(msg.type==="export-all-svg-page"||msg.type==="export-all-png-page"||msg.type==="export-all-svg-all-pages"||msg.type==="export-all-png-all-pages"){
    ...
    const pages=msg.type.includes("all-pages")?[...figma.root.children]:[figma.currentPage];let tN=0;for(const pg of pages)tN+=deepFlatten([pg]).filter((n:any)=>isVisible(n)&&n.type!=="PAGE").length;let pN=0;
    for(const pg of pages){const ns=deepFlatten([pg]).filter((n:any)=>isVisible(n)&&n.type!=="PAGE");
      for(let i=0;i<ns.length;i+=20){if(cancelRequested)break;const batch=ns.slice(i,i+20).map((n:any)=>n.id);const r=await exportNodes(batch,fmt,sc);
        for(const x of r){const ext=fmt.toLowerCase();const fname=pages.length>1?`${sanitizeName(pg.name)}/${x.name}.${ext}`:`${x.name}.${ext}`;zipFiles.push({name:fname,data:new Uint8Array(x.bytes)});}pN+=batch.length;figma.ui.postMessage({type:"progress",current:Math.min(pN,tN),total:tN,label:fmt});}}
