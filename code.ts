// Figma Extract All v7.0.0 — Deep recursive, robust, fast.
...
interface FullExtract {
  ...
  pages:ExtractedPage[];textNodes:ExtractedText[];variables:ExtractedVariable[];
  styles:ExtractedStyle[];components:ExtractedComponent[];
  nodeCounts:{total:number;textNodes:number;frames:number;components:number;instances:number;byType:{[t:string]:number}};
...
// Recursive flatten: returns ALL descendants INCLUDING the root node (if it has children)
function deepFlatten(roots:BaseNode[]):SceneNode[]{
  const results:SceneNode[]=[];
  function walk(n:BaseNode){
    if(n.type!=="PAGE"&&n.type!=="DOCUMENT")results.push(n as SceneNode);
...
function isExportableNode(n:SceneNode):boolean{return n.visible!==false&&ALL_ORIENT_TYPES.includes(n.type);}

// Fills
function extractFills(fills:ReadonlyArray<Paint>|typeof figma.mixed):FillInfo[]{if(fills===figma.mixed)return[];const r:FillInfo[]=[];for(const f of fills){if(f.type==="SOLID"&&f.color)r.push({type:"SOLID",hex:rgbToHex(f.color.r,f.color.g,f.color.b),rgba:{r:f.color.r,g:f.color.g,b:f.color.b,a:f.opacity??1},opacity:f.opacity??1,visible:f.visible??true,blendMode:f.blendMode??"NORMAL",boundVariableId:(f as any).boundVariables?.color?.id||null});else r.push({type:f.type,hex:"#000000",rgba:{r:0,g:0,b:0,a:1},opacity:f.opacity??1,visible:f.visible??true,blendMode:f.blendMode??"NORMAL",boundVariableId:null});}return r;}

// Text
function extractText(node:TextNode):ExtractedText{
  let lh:{value:number;unit:string}|null=null;const l=node.lineHeight;if(l!==figma.mixed&&typeof l==="object"&&"value" in l)lh={value:(l as any).value,unit:(l as any).unit||"PIXELS"};
  let ls:{value:number;unit:string}|null=null;const s=node.letterSpacing;if(s!==figma.mixed&&typeof s==="object"&&"value" in s)ls={value:(s as any).value,unit:(s as any).unit||"PIXELS"};
...
// Variables — robust with short timeout
async function extractAllVariables():Promise<ExtractedVariable[]>{
  const vars:ExtractedVariable[]=[];
  try{
    const timeout=new Promise<void>((_,rej)=>setTimeout(()=>rej(new Error("timeout")),3000));
    const localVars=await Promise.race([figma.variables.getLocalVariablesAsync(),timeout]) as Variable[];
    const modeMap:{[c:string]:{[m:string]:string}}={};
    try{const cols=await Promise.race([figma.variables.getLocalVariableCollectionsAsync(),timeout]) as VariableCollection[];for(const col of cols){modeMap[col.id]={};for(const m of col.modes)modeMap[col.id][m.modeId]=m.name;}}catch(e){/* continue without mode names */}
  ...
  }catch(e){/* timeout or API error — return empty */}
  return vars;
}
...
// Export SVGs — recursive, batches
async function exportNodeAsSVGEmbedded(nodeId:string):Promise<EmbeddedSVG|null>{
  const node=await figma.getNodeByIdAsync(nodeId);if(!node||node.type==="PAGE"||node.type==="DOCUMENT")return null;
...
async function exportAllSVGsForScope(roots:BaseNode[],onProgress?:(c:number,t:number)=>void):Promise<void>{
  ...
  const svgTargets=allNodes.filter(n=>n.type!=="TEXT"&&(n.type as string)!=="PAGE"&&(n.type as string)!=="DOCUMENT"&&isVisible(n));
  ...
  for(let i=0;i<svgTargets.length;i+=8){
    ...
    for(const r of results){if(r)figma.ui.postMessage({type:"download-file",fileName:`svgs/${sanitizeName(r.pageName)}/${r.nodeName}.svg`,content:r.svg,mimeType:"image/svg+xml"});}
...
async function exportNodes(nodeIds:string[],format:"SVG"|"PNG"|"JPG"|"PDF",scale:number):Promise<ExportResultItem[]>{
  ...
  for(const id of nodeIds){
    const node=await figma.getNodeByIdAsync(id);if(!node||node.type==="PAGE"||node.type==="DOCUMENT")continue;
    const sn=node as SceneNode;
...
// Lottie — RECURSIVE! Every child gets its own SVG entry
async function buildLottieBundle(roots:BaseNode[]):Promise<any>{
  const allNodes=deepFlatten(roots);
  const items:any[]=[];
  for(const n of allNodes){
    if((n.type as string)==="PAGE"||(n.type as string)==="DOCUMENT")continue;
    try{
      const s=await(n as SceneNode).exportAsync({format:"SVG_STRING"}as ExportSettingsSVGString);
      if(s&&s.length>10)items.push({id:n.id,name:sanitizeName(n.name),type:n.type,pageName:getPageName(n),width:(n as SceneNode).width,height:(n as SceneNode).height,svg:s});
    }catch(e){}
  }
...
figma.ui.onmessage=async(msg:any)=>{
  ...
  // FULL EXTRACT (plain, no opts)
  if(msg.type==="get-full-extract"&&!msg.aeOpts&&!msg.aiOpts){
    const data=await buildFullExtract((p:FullExtractProgress)=>{figma.ui.postMessage({type:"full-extract-progress",progress:p});});
  ...
  // AE EXTRACT = Full Extract + SVGs + Lottie (recursive!)
  if(msg.type==="get-full-extract"&&msg.aeOpts){
    ...
    if(msg.aeOpts.includeLottie&&!cancelRequested){
      const lottieItems=figma.currentPage.selection.length>0?figma.currentPage.selection.map(s=>s):[figma.currentPage as any];
    ...
    if(!cancelRequested)figma.ui.postMessage({type:"full-extract",data:{textNodes:data.textNodes.length,variables:data.variables.length,styles:data.styles.length,components:data.components.length,totalNodes:data.nodeCounts.total,scope:data.meta.scopeDescription}});
  }
  ...
  if(msg.type==="get-text"){
    const nodes=deepFlatten(scope.roots);const tn=nodes.filter(n=>n.type==="TEXT").map(n=>extractText(n as TextNode));
  ...
  if(msg.type==="export-lottie-json"){
    const sel=figma.currentPage.selection;if(sel.length===0){figma.ui.postMessage({type:"error",message:"No nodes selected"});return;}
    const bundle=await buildLottieBundle([...sel]);figma.ui.postMessage({type:"download-file",fileName:bundle.fileName,content:JSON.stringify(bundle,null,2),mimeType:"application/json"});
  }
  ...
  if(msg.type==="export-all-svg-page"||msg.type==="export-all-png-page"||msg.type==="export-all-svg-all-pages"||msg.type==="export-all-png-all-pages"){
    ...
    for(const pg of pagesToExport){totalN+=deepFlatten([pg]).filter(isExportableNode).length;}
